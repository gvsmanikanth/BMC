<?php
	$pageTitle = '';
	$bodyClass = '';
	include_once 'php-inc/head.php';
?>



<section class="quote-section js-eh">
<div class="layout-inner-wrap">
	<h2>See why customers love TrueSight Operations Management </h2><br>
</div>
	<div class="owl-carousel-quote ">
		<div class="item ">
			<div class="quote js-ehItem">
				<div class="star-ratings-sprite"><span style="width:100%" class="star-ratings-sprite-rating"></span></div>
				<h4>Monitors diverse set of hardware products from multiple manufacturers</h4>
				<hr>
				<p>It has completely changed our way of thinking about our strategy going forward.
                 It's amazing. At this point in time, I'd rate it a ten out of ten. <a href="//www.itcentralstation.com/product_reviews/bmc-truesight-operations-management-review-62335-by-paul-mercina" 
                 class="more" target="_blank">More</a>
				</p>
				<div class="quote-footer flex-wrap">
					<div class="info">
						<p><strong>Paul Mercina</strong></p>
						<p>Director, Product Management</p>
					</div>
					<div class="logo">
						<img src="/front-end/Assets/src/img/ITCS-logo.png" alt="ITCS-logo" style="max-width:150px">
					</div>
				</div>
			</div>
		</div> 
		<div class="item ">
			<div class="quote js-ehItem">
				<div class="star-ratings-sprite"><span style="width:90%" class="star-ratings-sprite-rating"></span></div>				
				<h4>The solution's event management capabilities are fantastic</h4>
				<hr>
				<p>The solution provides us with a single pane of glass where we can ingest data and events from many technologies.
                <a href="//www.itcentralstation.com/product_reviews/bmc-truesight-operations-management-review-65257-by-doug-greene" class="more" target="_blank">More</a>
				</p>
				<div class="quote-footer flex-wrap">
					<div class="info">
						<p><strong>Doug Greene</strong></p>
						<p>Sr. Director, Operations</p>
					</div>
					<div class="logo">
						<img src="/front-end/Assets/src/img/ITCS-logo.png" alt="ITCS-logo" style="max-width:150px">
					</div>
				</div>
			</div>
		</div> 
		<div class="item ">
			<div class="quote js-ehItem">
				<div class="star-ratings-sprite"><span style="width:90%" class="star-ratings-sprite-rating"></span></div>		
				<h4>Monitoring all the way from storage up to the servers</h4>
				<hr>
				<p>Its event management capabilities are very open and flexible. 
                You can solve problems as far as your imagination can go with it. <a href="//www.itcentralstation.com/product_reviews/bmc-truesight-operations-management-review-62971-by-george-klarmann" class="more" target="_blank">More</a>
				</p>
				<div class="quote-footer flex-wrap">
					<div class="info">
						<p><strong>George Klarmann</strong></p>
						<p>CEO at Transcendence IT</p>
					</div>
					<div class="logo">
						<img src="/front-end/Assets/src/img/ITCS-logo.png" alt="ITCS-logo" style="max-width:150px">
					</div>
				</div>
			</div>
		</div>
		<div class="item ">
			<div class="quote js-ehItem">
				<div class="star-ratings-sprite"><span style="width:90%" class="star-ratings-sprite-rating"></span></div>		
				<h4>Monitoring all the way from storage up to the servers</h4>
				<hr>
				<p>Its event management capabilities are very open and flexible. 
                You can solve problems as far as your imagination can go with it. <a href="//www.itcentralstation.com/product_reviews/bmc-truesight-operations-management-review-62971-by-george-klarmann" class="more" target="_blank">More</a>
				</p>
				<div class="quote-footer flex-wrap">
					<div class="info">
						<p><strong>George Klarmann</strong></p>
						<p>CEO at Transcendence IT</p>
					</div>
					<div class="logo">
						<img src="/front-end/Assets/src/img/ITCS-logo.png" alt="ITCS-logo" style="max-width:150px">
					</div>
				</div>
			</div>
		</div>
				   
	</div>
</section>





<?php include_once 'php-inc/foot.php'; ?>